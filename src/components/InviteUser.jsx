...
export default function InviteUser({ workspaceId }) {
  ...
  const AdminBadge = () => (
    <motion.div 
      ...
      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100"
    >
      <HiOutlineShieldCheck className="w-4 h-4 text-blue-600" />
      <span className="text-xs font-medium text-blue-700">Workspace Admin</span>
    </motion.div>
  );
      ...
      >
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 p-3 bg-amber-50 border border-amber-100 rounded-xl">
            <HiOutlineShieldCheck className="w-5 h-5 text-amber-500 flex-shrink-0" />
            <div>
              <p className="text-sm font-medium text-amber-800">Admin Access Required</p>
              <p className="text-xs text-amber-600">Only workspace admins can invite new members</p>
            </div>
          </div>
        </div>
      </motion.div>
            ...
            >
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="block w-full pl-10 pr-4 py-4 border border-gray-200 rounded-xl bg-white/50 focus:ring-2 focus:ring-blue-500 focus:border-transparent focus:outline-none transition-all"
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="w-full py-4 px-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-medium rounded-xl shadow-lg hover:shadow-xl disabled:opacity-70 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center gap-2"
              >
                {isLoading ? (
                  <>
                    <LoadingSpinner />
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto pr-2">
                    {invitedUsers.map((user, index) => (
                      <motion.div
                        ...
                        className="flex items-center justify-between p-3 bg-gradient-to-r from-gray-50 to-white border border-gray-100 rounded-lg hover:border-blue-100 transition-colors"
                      >
                        <div className="flex items-center gap-3">
